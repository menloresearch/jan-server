...
// Helper functions (now mostly deprecated as logic moved to ModelService)

func getAibrixStatus(available bool) string {
	if available {
		return "Aibrix PodAutoscaler CRD is installed and ready"
	}
	return "Aibrix PodAutoscaler CRD is missing - required for model autoscaling"
}

func getGPUOperatorStatus(available, hasGPUs bool) string {
	if !hasGPUs {
		return "No GPU nodes detected in cluster"
	}
	if available {
		return "GPU Operator is installed and GPU resources are exposed"
	}
	return "GPU Operator is missing - required for GPU workloads"
}

func getKuberayStatus(available bool) string {
	if available {
		return "KubeRay Operator is installed and ready"
	}
	return "KubeRay Operator is missing - required for Ray-based inference"
}

func getEnvoyGatewayStatus(available bool) string {
	if available {
		return "Envoy Gateway is installed and ready"
	}
	return "Envoy Gateway is missing - required for HTTP routing"
}

// Note: Storage and namespace validation functions removed - logic moved to ModelService

func (api *KubernetesAPI) generateValidationMessages(response *ClusterValidationResponse) {
	// Generate warnings for optional features
	if !response.Dependencies.GPUOperator.Available && response.GPUStatus.HasGPUs {
		response.Warnings = append(response.Warnings, "GPU Operator not detected but GPU nodes are present")
	}

	if !response.Dependencies.StorageClasses.Available {
		response.Warnings = append(response.Warnings, "No default storage class - PVC-based model caching will not work")
	}

	// Generate errors for required features
	if !response.Dependencies.AibrixOperator.Available {
		response.Errors = append(response.Errors, "Aibrix PodAutoscaler is required for model deployment")
	}

	if !response.Dependencies.KuberayOperator.Available {
		response.Errors = append(response.Errors, "KubeRay Operator is required for distributed inference")
	}

	if !response.Dependencies.EnvoyGateway.Available {
		response.Errors = append(response.Errors, "Envoy Gateway is required for HTTP routing")
	}
}

// Note: GPU calculation functions removed - logic moved to ModelService with Redis caching for better performance
